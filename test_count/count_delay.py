...
#    root='/home/zc/trace/'+file_name
    ...
    for file in trace_files:
        ...
        for request in requests:
            if int(request[delay]*1000)<5:
                size_list[0] += 1
            elif int(request[delay]*1000)<20:
                size_list[1] += 1
            elif int(request[delay]*1000)<50:
                size_list[2] += 1
            elif int(request[delay]*1000)<100:
                size_list[3] += 1
            elif int(request[delay]*1000)<500:
                size_list[4] += 1
            elif int(request[delay]*1000)<1000:
                size_list[5] += 1
            elif int(request[delay]*1000)<2000:
                size_list[6] += 1
            elif int(request[delay]*1000)<5000:
                size_list[7] += 1
            elif int(request[delay]*1000)<10000:
                size_list[8] += 1
            elif request[size]<50000:
                size_list[9] += 1
        j += 1
    with open('delay_'+file_name+'.txt', 'w') as f:
        f.write(str(j)+'\n')
